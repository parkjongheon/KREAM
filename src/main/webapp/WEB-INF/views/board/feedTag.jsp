...
<div class="w3-top w3-white" style="margin-top: 100px;height: 70px">
  <div class="w3-white w3-center" style="margin-left: 32%">
		<div class="w3-container w3-center" style="width: 700px">
		<h2 style="font-weight: bold">
			<span style="text-align: center;font-size: 24px;letter-spacing: -.12px;white-space: nowrap;">#${param.tags}</span>
		</h2>
		</div>
  </div>
</div>
		...
		<div class="w3-container" style="width:1280px;margin: auto;padding-top:10px;padding: auto">
			<div class="w3-animate-opacity w3-center" style="height: 100%;width: 1280px">
				<div class="w3-container w3-center" style="width: 1280px">
					<a href="${ctp}/board/feedAll?pag=1&pageSize=16" class="w3-button w3-black w3-hover-black w3-round-xlarge"><i class="fa fa-arrow-left" aria-hidden="true"></i> 전체 피드</a>
					<c:forEach var="tvo" items="${tvos}">
					<a href="${ctp}/board/feedTag?pag=1&pageSize=16&tags=${tvo.tagName}" class="w3-button w3-white w3-hover-white w3-round-large tag w3-margin-left">#${tvo.tagName }</a>
					</c:forEach>
				</div>
			  ...
			  <a href="${ctp}/board/feedTag?pag=1&pageSize=${pagevo.pageSize}&tags=${param.tags}" class="w3-button w3-xlarge">&laquo;</a>
			  </c:if>
			  <c:if test="${pagevo.pag == 1}">
			  <a class="w3-button w3-xlarge w3-disabled">&laquo;</a>
			  </c:if>
			  <c:if test="${pagevo.curBlock > 1 }">
			  <a href="${ctp}/board/feedTag?pag=${(pagevo.curBlock-1)*pagevo.blockSize+1}&pageSize=${pagevo.pageSize}&tags=${param.tags}" class="w3-button">&laquo;</a>
			  </c:if>
				...
				<c:forEach var="i" begin="${no}" end="${size}">
					<c:choose>
						...
						<c:when test="${i == pagevo.pag}">
							<a href="${ctp}/board/feedTag?pag=${i}&pageSize=${pagevo.pageSize}&tags=${param.tags}" class="w3-button w3-theme-l4">${i}</a>
						</c:when>
						<c:otherwise>
							<a href="${ctp}/board/feedTag?pag=${i}&pageSize=${pagevo.pageSize}&tags=${param.tags}" class="w3-button">${i}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			  <c:if test="${pagevo.curBlock < pagevo.lastBlock}">
					<a href="${ctp}/board/feedTag?pag=${(curBlock+1)*blockSize+1}&pageSize=${pageSize}&tags=${param.tags}" class="w3-button">&raquo;</a>	
				</c:if>
				<c:if test="${pagevo.pag != pagevo.totPage}">
					<a href="${ctp}/board/feedTag?pag=${pagevo.totPage}&pageSize=${pagevo.pageSize}&tags=${param.tags}" class="w3-button w3-xlarge">&raquo;</a>
				</c:if>
				<c:if test="${pagevo.pag == pagevo.totPage}">
