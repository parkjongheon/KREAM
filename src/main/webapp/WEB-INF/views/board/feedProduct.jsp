...
<div class="w3-top w3-white" style="margin-top: 100px;height: 60px">
	<div class="w3-white" style="margin-left: 42%">
   <div class="w3-container w3-row" style="width: 700px">
   		<div class="w3-col m1">
   			<c:choose>
		    	<c:when test="${pvo.ebrName == 'Nike'}">
		    		<img class="w3-round-large" src="${ctp}/product/${pvo.prdfName}" alt="Card image" style="width: 50px;height: 50px;background-color: #EBF0F5">
		    	</c:when>
		    	<c:when test="${pvo.ebrName == 'Jordan'}">
		    		<img class="w3-round-large" src="${ctp}/product/${pvo.prdfName}" alt="Card image" style="width: 50px;height: 50px;background-color: #F6EEED">
		    	</c:when>
		    	<c:when test="${pvo.ebrName == 'Adidas'}">
		    		<img class="w3-round-large" src="${ctp}/product/${pvo.prdfName}" alt="Card image" style="width: 50px;height: 50px;background-color: #F1F1EA">
		    	</c:when>
		    	<c:otherwise>
		    		<img class="w3-round-large" src="${ctp}/product/${pvo.prdfName}" alt="Card image" style="width: 50px;height: 50px;background-color: #F5F5F5">
		    	</c:otherwise>
		    </c:choose>
   		</div>
   		<div class="w3-col m11">
   			<h3 style="margin-top: 5px;">${pvo.eprdName}</h3>
   		</div>
		</div>
	</div>
</div>
	...
	<div class="w3-container" style="width:100%;height: 100%">
		<div class="w3-container" style="width:1280px;margin: auto;padding-top:10px;padding: auto">
			<div class="w3-animate-opacity w3-center" style="height: 100%;width: 1280px">
			  ...
			  <a href="${ctp}/board/feedProduct?pag=1&pageSize=${pagevo.pageSize}&prdIdx=${param.prdIdx}" class="w3-button w3-xlarge">&laquo;</a>
			  </c:if>
			  <c:if test="${pagevo.pag == 1}">
			  <a class="w3-button w3-xlarge w3-disabled">&laquo;</a>
			  </c:if>
			  <c:if test="${pagevo.curBlock > 1 }">
			  <a href="${ctp}/board/feedProduct?pag=${(pagevo.curBlock-1)*pagevo.blockSize+1}&pageSize=${pagevo.pageSize}&prdIdx=${param.prdIdx}" class="w3-button">&laquo;</a>
			  </c:if>
				...
				<c:forEach var="i" begin="${no}" end="${size}">
					<c:choose>
						...
						<c:when test="${i == pagevo.pag}">
							<a href="${ctp}/board/feedProduct?pag=${i}&pageSize=${pagevo.pageSize}&prdIdx=${param.prdIdx}" class="w3-button w3-theme-l4">${i}</a>
						</c:when>
						<c:otherwise>
							<a href="${ctp}/board/feedProduct?pag=${i}&pageSize=${pagevo.pageSize}&prdIdx=${param.prdIdx}" class="w3-button">${i}</a>
						</c:otherwise>
					</c:choose>
				</c:forEach>
			  <c:if test="${pagevo.curBlock < pagevo.lastBlock}">
					<a href="${ctp}/board/feedProduct?pag=${(curBlock+1)*blockSize+1}&pageSize=${pageSize}&prdIdx=${param.prdIdx}" class="w3-button">&raquo;</a>	
				</c:if>
				<c:if test="${pagevo.pag != pagevo.totPage}">
					<a href="${ctp}/board/feedProduct?pag=${pagevo.totPage}&pageSize=${pagevo.pageSize}&prdIdx=${param.prdIdx}" class="w3-button w3-xlarge">&raquo;</a>
				</c:if>
				<c:if test="${pagevo.pag == pagevo.totPage}">
