	...
	<div class="w3-container" style="width:100%;height: 100%;margin-bottom: 50px">
		<div class="w3-container" style="width:1400px;margin: auto;padding-top:10px;padding: auto">
			<div class="w3-animate-opacity w3-center" style="height: 100%;width: 1400px">
				<hr>
				<form name="myForm" method="post" onsubmit="return false" enctype="multipart/form-data">
					...
					<div class="w3-row" >
						<div class="w3-col m6">
							...
							<c:set var="fNames" value="${fn:split(bvo.bo_fName,'/')}" />
								...
								<input type="hidden" id="hImage0" name="hImage" value="${fNames[0]}">
								</div>
							</div>
							<div class="w3-right" style="width: 600px;height: 700px;margin-right: 40px;padding: 0px">
					  		<img class="w3-border w3-round-large" src="${ctp}/board/${fNames[0]}" style="width: 600px;height: 700px"  id="view" alt="미리보기"/>
							</div>
							...
							<div id="imgBox" class="w3-margin-top w3-right" style="width:600px;height: 100px;margin-right: 40px;border-color: gray">
					  		<c:forEach var="i" begin="1" end="${fn:length(fNames)-1}">
					  		<div class="notification" id="notifi${i}">
					  		<img class="w3-border w3-round-large" src="${ctp}/board/${fNames[i]}" style="width:100px;height: 100px" id="view${i}" alt="미리보기"/>
					  		<a href="javascript:delhImage(${i})">
							...
							<c:forEach var="i" begin="1" end="${fn:length(fNames)-1}">
								<input type="hidden" id="hImage${i}" name="hImage" value="${fNames[i]}">
								<input type="file" name="file" id="fName${i}"/>
							</c:forEach>
							</c:if>							
								<input type="hidden" name="originalfNames">					
							</div>
						</div>
						<div class="w3-col m6">
							<ul class="w3-ul">
							...
							<textarea class="w3-border w3-padding w3-round-large" rows="8" cols="70" style="resize: none" id="bo_content" name="bo_content" placeholder="회원님의 사진을 표현해 보세요">${bvo.bo_content}</textarea>
							<div class="w3-row">
								<div class="w3-col m6">
							...
							<div class="w3-row" style="height: 100px">
								<div class="w3-col">
									...
									<div class='tag w3-light-gray w3-round-large' id='tagItem${i}' style='margin-bottom : 5px'>
										<span><b><font color='gray'>${tags[i]}</font></b></span>
										<a style='margin-left : 10px' href='javascript:deltag(${i})'>
										<i class='fa fa-times' aria-hidden='true'></i>
										</a>
									<input type="hidden" name="tagH" id="tagH${i}" value="${tags[i]}">
									</div>
									</c:forEach>
									
									</div>
									<input type="hidden" class="w3-input" id="hiddenTag" name="bo_tag" value="${bvo.bo_tag }">
								</div>
							</div>
							<div class="w3-row">
								<div class="w3-col">
							...
							<div class="w3-row w3-margin-top">
								...
								<c:if test="${bvo.bo_prdIdx != 0}">
									<div class="card w3-left" id="prdTagItem" style="height: 250px;width: 150px;margin-right: 20px;margin-bottom:10px">
									<a href="javascript:delPrdTage()"><span class="badges">x</span></a>
									...
									<input type="hidden" id="prdTag" name="bo_prdIdx" value="${bvo.prdIdx}">
									</div>
									</div>
								</c:if>
								</div>
								<input type="hidden" name="boIdx" value="${bvo.boIdx}">
							</div>
						</div>
					</div>
				</div>
				...
				<hr>
				<div class="w3-center">
					<a href="javascript:updateCheck()" class="w3-button w3-border">등록하기</a>
				</div>
			</div>
		</div>
	</div>
